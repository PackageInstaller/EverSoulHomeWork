  ...
  if (process.env.NEXT_RUNTIME === 'nodejs' || !process.env.NEXT_RUNTIME) {
    ...
    try {
      ...
      if (cacheStats.totalEntries === 0) {
        console.log('📦 [服务器启动] 缓存为空，开始预加载游戏数据...');
        ...
        console.log(`✅ [服务器启动] 缓存已存在，跳过预加载 (${cacheStats.totalEntries} 项数据)`);
        const hitRate = cacheStats.cacheHits + cacheStats.cacheMisses > 0 
          ? (cacheStats.cacheHits / (cacheStats.cacheHits + cacheStats.cacheMisses) * 100).toFixed(2)
          : '0.00';
        console.log(`📊 [服务器启动] 缓存命中率: ${hitRate}%`);
      }
      
    } catch (error) {
      console.error('❌ [服务器启动] 游戏数据缓存检查失败:', error);
